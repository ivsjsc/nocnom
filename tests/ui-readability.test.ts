...
assert(health.includes('text-sm font-black text-amber-950'), 'Health warning heading uses strong foreground contrast');
assert(health.includes("Chưa có số đo") && health.includes('border border-slate-300 bg-slate-100'), 'Missing BMI measurement is rendered as a clear chip');
assert(health.includes('grid grid-cols-4 gap-1 text-[10px] font-extrabold'), 'BMI range labels use a legible grid hierarchy');
